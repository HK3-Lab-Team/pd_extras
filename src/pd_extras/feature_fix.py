...
def convert_maps_from_tuple_to_str(group_id_to_tuple_map: Dict[str, Tuple]):
    """
    ...
    -------
    Dict[str, str]
        Dictionary with the same keys as ``group_id_to_tuple_map``, but where 
        the related values have been transformed from tuple to string.
    """
    gr_id_to_string_map = {}
...
def _one_hot_encode_column(
    ...
    -------
    pd.DataFrame
        DataFrame similar to ``df`` with th additional columns generated by 
        OneHotEncoder encoder.
    sklearn.preprocessing.OneHotEncoder
        OneHotEncoder instance that has been used for encoding and that could be
        used for reverting the operation.
    Tuple[str]
        Tuple of the column names that have been created with encoded values.
...
def _ordinal_encode_column(
    df, column, drop_old_column: bool = False
) -> Tuple[pd.DataFrame, sklearn.preprocessing.OrdinalEncoder, Tuple[str]]:
    """
    Encode categorical column named ``column`` using OrdinalEncoder
    ...
    sklearn.preprocessing.OrdinalEncoder
        OrdinalEncoder instance that has been used for encoding and that could be
        used for reverting the operation.
    Tuple[str]
        Tuple of the column names that have been created with encoded values.
