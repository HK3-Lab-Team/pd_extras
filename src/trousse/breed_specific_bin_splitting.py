...
def get_increasing_thresholds_for_bin_splitting(
    ...
    """
    This splits the interval [range_min, range_max] in bins that increase by
    range_increase. So it returns the thresholds computed as:
    '''
    bin[i] = range_increase * bin[i-1]
    threshold[i] = threshold[i-1] + bin[i]
    '''
    The bin[0] is computed so that the final number of bins (following that rule)
    is 'bin_count' and it covers the whole range following the rules above.
    ...
    bin_thresh_increase: float
        How much the next bin is increased w.r.t the previous bin
        (bin[i] = range_increase * bin[i-1])
    bin_count:
    ...
    bin_size_list: List
        List of bin sizes (i.e. threshold[i+1] - threshold[i]). It can be useful if
        reused in later computations (without recalculating)
...
def get_bin_upp_low_value_from_thresholds(
    ...
    """
    The function will compute age bins adding some extra values (overlapped) in every
    age bin in order to limit threshold effects.
    E.g. extra_bin_size = 0.5 -> The bin used for the samples in range
        [threshold[i], threshold[i+1]] will be:
            [ (threshold[i] - bin_size[i-1] * 0.5)  ;
            (threshold[i+1] + bin_size[i+1] * 0.5) ]

    Parameters
    ----------
    extra_bin_size: float
        This indicates how much extra values will be considered in the age_bin. This is
        the portion of next/previous bin that will be included in the bin. If set to 0,
        the age bin will be calculated as interval between two thresholds [threshold[i],
        threshold[i+1]].
    thresh_list: Tuple
        Tuple of the age bin thresholds (that separate the age bins). They must include
        minimum and maximum value if you want to include all the data
    bin_sizes: Tuple[Tuple]
        These are just the intervals between two thresholds [threshold[i],
        threshold[i+1]]. If not provided, they will be computed based on thresh_list.
        This is for performances sake. because when provided this computation will not
        be required (it may have been computed during bin_thresholds computation by
        '_split_range_increasing_thresholds' function. Default set to None.
    ...
    # Add the last bin with very large number as upper value in order to include every
    # older patients from future dataset (test_set)
    bins_list.append(
...
def get_bin_list_per_single_breed(
    ...
    """
    This function computes threshold values for bin splitting for a single breed
    (df_breed). Then it uses these to compute the bin upper and lower values.
    ...
    start_from_zero: bool
        Option to force the age range to start from 0 and not from the 'column_to_split'
        minimum value
    bin_thresholds: Tuple[float]
        This is an optional argument to manually force some thresholds for the age bin
        splitting. If provided, the bins are calculated based on these
        (bin_size_increase is still required). A bin will be associated only to the data
        points that are in the range between the first value and the last value
    sample_count_threshold: int
        When computing the range of the age values, some breeds may have too few samples
        to make a reliable range estimation. So this value indicates the sample count
        threshold under which the mongrel's age range will be used.
    ...
    if df_breed.shape[0] < sample_count_threshold:
        if mongrels_age_bins == ():
            raise ValueError(
                ...
                f"{df_breed['BREED'].unique()[0]} has too few samples "
                f"(<{sample_count_threshold}) so the age range computed would not "
                "be reliable"
            )
        else:
...
def create_df_with_overlapping_bins_single_breed(
    ...
    """
    It splits the samples from DataFrame 'df_breed' into age bins according to bins_list
    definition. The bins_list may define overlapping bins and in this case overlapping
    rows from different age bins are repeated so that a groupby function can be applied
...
def add_column_bins_to_single_breed(
    ...
    """
    # TODO: Check if bins list contains overlapping bins. In that case we need to create
    # a new DataFrame
    col_array = df_breed[column_to_split].values
    # Instantiate an empty DataFrame with the new column added
    for id_bin, bin_range in enumerate(bins_list):
...
def _apply_compute_age_bin_per_breed(
    ...
    """
    This function is supposed to be used in a '.apply()' function from
    'add_breed_specific_age_bin' in order to compute age_bin for each breed (df_breed)
    ...
    column_to_split: str
        Name of the column whose values we want to split into bins
    new_column_name: str
        Name of the new column that will contain the new bin identifiers (the encoded
        values)
    column_bin_to_range_map_per_breed: Dict
        This Dict will contain the map between the breed + related bin_ids and the range
        of values in bin. This is populated through the function calls by .apply() so
        that each breed is mapped.
    mongrels_age_bins:
        These are the age_bins computed for mongrel breed and they will be used for the
        less populated breeds so the age range is more reliable
    bin_thresh_increase: float
    ...
    -------
    pandas.DataFrame
        The same df_breed data with an additional column containing the
        breed_specific_age_bin
    """
...
def _get_samples_with_breed_not_nan(
    ...
    """
    Find the samples with NaN breed values and drop them (they will be reinserted after
    computation)
...
def add_breed_specific_bin_id_to_df(
    ...
    """
    This function adds an extra column containing the bin identifier for the
    'column_to_split', computed as follows.
    First, the dataset is split according to breed. Then for each breed we look for the
    range of age values and we split the range into bins according to the arguments
    provided. We finally associate the row to the age_bin id according to its age value
    ...
    bin_thresholds: Tuple[float]
        ...
        splitting. If provided, the bins are calculated based on these
        (bin_size_increase is still required). A bin will be associated only to the data
        points that are in the range between the first value and the last value. The
        thresholds will be used as the lowest value of the bin.
    sample_count_threshold: int
        When computing the range of the age values, some breeds may have too few samples
        to make a reliable range estimation. So this value indicates the sample count
        threshold under which the mongrel's age range will be used.
    ...
    -------
    Dataset
        New Dataset with the additional column. The new column is defined as global
        variable AGE_BIN_COLUMN_BREED
    Dict[str, Dict]
    ...
    # Compute the bins from the most populated breed so that the least populated can use
    # these bins when lacking infos about the range
    ...
    # For each breed compute the appropriate age_bins and add the new column
    # AGE_BIN_COLUMN_BREED containing the age_bins that the row belongs to
    ...
    # Create Dataset with same instance attribute as dataset, but with the new
    # bin_column
