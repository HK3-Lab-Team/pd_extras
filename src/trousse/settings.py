...
# When we check a column with only string values in order to see if the strings are
# actually numeric values, we try to cast string to numeric and we will get NaN if the
# values are not castable to numeric. If the ratio of
# "not-NaN values after conversion"/ "not-NaN values before conversion to numeric" >
#     NOT_NA_STRING_COL_THRESHOLD --> the column is considered to be numeric and later
...
# Map that replaces these words (keys) with their values when a datum is exactly
# identical to that key
