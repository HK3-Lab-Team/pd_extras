...
def is_subgroup_populated(subgroup, sample_count_threshold_for_statistics, feature_list):
...
def subgroups_statistical_test(df_group_slice, subgroup_col_name, groupby_column, separation_count,
                               full_plot_list, feature_list,
                               H_value_by_group_by_subgroup, p_value_by_group_by_subgroup,
                               sample_count_threshold_for_statistics=20, p_value_stat_func=ss.kruskal,
                               p_value_thresholds=(0.05, 0.01), complete_df_info: DataFrameWithInfo = None,
    ...
    if complete_df_info is None and show_separated_boxplot:
        logging.error('If you want to see the most separated boxplot, you must provide the original '
    ...
    unique_subgroup_values = sorted([x[0] for x in df_groupby_subgroups[subgroup_col_name].unique()])
    possible_combination_subgroups = list(itertools.combinations(unique_subgroup_values, 2))
    possible_combination_subgroups.sort()

    # Create an array with the group_name and every possible subgroup combination
    p_value_index_array = np.array([[group_name] * len(possible_combination_subgroups),
                                    possible_combination_subgroups]).transpose()
    # Create zero array for p_value related to every feature
    p_value_feat_array = np.full(shape=(len(possible_combination_subgroups), len(feature_list)), fill_value=-1,
                                 dtype=int)
    ...
    H_value_by_group_by_subgroup[group_name], p_value_by_group_by_subgroup[group_name] = {}, {}
    ...
    for feat in feature_list:
        ...
        for g1_id, g2_id in possible_combination_subgroups:
            ...
            H_value_by_group_by_subgroup[group_name][pair], p_value_by_group_by_subgroup[group_name][pair] = {}, {}
            ...
            try:
                is_subgroup_populated_bool = is_subgroup_populated_bool_map.at[g1_id, feat] and \
                                             is_subgroup_populated_bool_map.at[g2_id, feat]
            ...
            if is_subgroup_populated_bool:
                ...
                # correct combination
                group_1_distribution = complete_df_info.df.loc[groupby_subgroups_ids[g1_id]][feat].dropna().values
                group_2_distribution = complete_df_info.df.loc[groupby_subgroups_ids[g2_id]][feat].dropna().values
                ...
                try:
                    ...
                    if ss.kruskal:
                        kruskal_result = p_value_stat_func(group_1_distribution, group_2_distribution)
                        p_value_by_group_by_subgroup[group_name][pair][feat] = kruskal_result.pvalue
                        ...
                        H_value_by_group_by_subgroup[group_name][pair][feat], \
                            p_value_by_group_by_subgroup[group_name][pair][feat] = p_value_stat_func(pair_data).at[2, 1]
                    else:
                        raise NotImplementedError(f"The function {p_value_stat_func} is not implemented yet. The "
                    ...
                    if 'identical' in str(e):
                        ...
                        logging.info(f'Every value of the subgroups {g1_id} and {g2_id} for feature {feat} ' \
                ...
                p_value_interval_label = len(p_value_thresholds) - bisect.bisect_right(p_value_thresholds,
                                                                                       p_value_by_group_by_subgroup[
                                                                                           group_name][pair][feat])
                ...
                p_value_separation_map.loc[
                    p_value_separation_map[SUBGROUP_PAIR_COMBINATION_COLUMN] == pair, feat] = p_value_interval_label

                if p_value_by_group_by_subgroup[group_name][pair][feat] < show_separated_boxplot_params[1]:
                ...
                if show_separated_boxplot and (feat_separation_counter > show_separated_boxplot_params[0]) \
                        and (feat not in features_already_plotted):

                    # Create a new plot for every level
                    p, _ = make_boxplot(complete_df_info,
...
def draw_and_save_plot_list(plot_list, file_name=None, column_num_in_grid=3):
    # make a grid
    grid = bk.gridplot(plot_list, ncols=column_num_in_grid, plot_width=500, plot_height=500)
...
def draw_every_plot_from_apply(full_plot_list):
    for breed in full_plot_list.keys():
        for part_plot_list_id in range(len(full_plot_list[breed])):
            draw_and_save_plot_list(full_plot_list[breed][part_plot_list_id],
                                    file_name=os.path.join(
                                        f'{breed}_Kruskal_most_separated_classes_{part_plot_list_id}.html'))


def compute_separation_per_breed_per_subgroup_vs_feature(df_info: DataFrameWithInfo, groupby_column: str,
                                                         ...
                                                         feature_list=None, show_separated_boxplot=False,
                                                         show_separated_boxplot_params=(30, 0.01), filename=None):
    """
    ...
    if first_groups_sorted is None:
        ...
        df_samples_per_group = get_show_samples_per_group(df_info, groupby_column, show_plot=False)
        # Select only first elements
        first_groups_sorted = list(df_samples_per_group.loc[:first_group_elements_threshold][groupby_column])

    # Select only the top populated groups based on first_groups_sorted
    df_most_populated_groups = df_info.df[df_info.df[groupby_column].isin(first_groups_sorted)]
    # Create groups
    ...
    p_value_thresholds = sorted(p_value_thresholds, reverse=False)  # Possible p-value thresholds
    ...
    p_value_separation_map = df_groupby_slice.apply(
        ...
        subgroup_col_name=subgroup_col_name, feature_list=feature_list,
        groupby_column=groupby_column, separation_count=separation_count,
        complete_df_info=df_info,
        sample_count_threshold_for_statistics=sample_count_threshold_for_statistics,
        full_plot_list=full_plot_list, p_value_stat_func=p_value_stat_func,
        show_separated_boxplot=show_separated_boxplot,
        show_separated_boxplot_params=show_separated_boxplot_params,
    ...
    if show_separated_boxplot:
        logging.info(f'Plotting the boxplots for the top {first_group_elements_threshold} most '
    ...
    p_value_separation_map = p_value_separation_map.set_index([groupby_column, SUBGROUP_PAIR_COMBINATION_COLUMN])
...
if __name__ == "__main__":
    ...
    p_value_separation_map = \
        compute_separation_per_breed_per_subgroup_vs_feature(
            df_info=df_info,
    ...
    p_value_age_names = p_value_age_per_feature.index.values[np.argsort(p_value_age_per_feature.values)][::-1]

    print(p_value_age_names[:23])
    separated_feat = [
        'percentage_vessel_right', 'vessel_volumes_mean',
        'ggo_volumes_mean', 'percentage_ggo_right',
